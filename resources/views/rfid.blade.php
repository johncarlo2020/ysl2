...
@section('content')
    <div class="mt-4 row">
        ...
        <div class="mb-4 col-lg-12">
            <div class="card">
                <div class="p-3 pb-0 card-header">
                    ...
                    <small class="text-muted">Open each link on the dedicated device at that station and leave it open. The
                        RFID reader must be plugged into that device.</small>
                </div>
                <div class="d-flex flex-wrap gap-3 pt-3 card-body">
                    @foreach (\App\Models\Station::orderBy('id')->get() as $station)
                        <a href="{{ route('kiosk', $station->id) }}" target="_blank"
                            class="d-flex align-items-center gap-2 btn-outline-dark btn">
                            <i class="fa-solid fa-id-card"></i>
                            Station {{ $station->id }}: {{ $station->name }}
                            <i class="fa-arrow-up-right-from-square ms-1 fa-solid" style="font-size:0.7rem;"></i>
                        </a>
                    @endforeach
                </div>
            </div>
        ...
        <div class="mb-4 mb-lg-0 col-lg-12">
            <div class="card">
                <div class="d-flex flex-wrap align-items-center justify-content-between gap-2 p-3 pb-0 card-header">
                    <div>
                        <h6 class="mb-0">RFID Card Assignment</h6>
                        <small class="text-muted">Tap an RFID card on the reader or type the UID manually to assign it to a
                            user.</small>
                    </div>
                    <button class="btn-outline-secondary btn btn-sm" id="check-card-btn" data-bs-toggle="modal"
                        data-bs-target="#checkCardModal">
                        <i class="me-1 fa-solid fa-magnifying-glass"></i> Check Card
                    </button>
                </div>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <span class="rfid-display-{{ $user->id }}">
                                            @if ($user->rfid_uid)
                                                <span class="bg-success badge">{{ $user->rfid_uid }}</span>
                                            @else
                                    ...
                                    <td>
                                        <button class="btn btn-sm btn-primary assign-btn" data-user-id="{{ $user->id }}"
                                            data-user-code="{{ $user->code }}" data-bs-toggle="modal"
                                            data-bs-target="#assignModal">
                                            <i class="me-1 fa-solid fa-id-card"></i>
                                            {{ $user->rfid_uid ? 'Re-assign' : 'Assign' }}
                                        </button>
                                        @if ($user->rfid_uid)
                                            <button class="ms-1 btn btn-sm btn-danger unlink-btn"
                                                data-user-id="{{ $user->id }}" data-user-code="{{ $user->code }}">
                                                <i class="me-1 fa-solid fa-link-slash"></i>Unlink
                                            </button>
                                        @endif
                                    </td>
                                </tr>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p class="mb-3 text-muted text-sm">Tap the RFID card on the reader connected to this PC to check if it
                        is already linked to a user.</p>
                    <div class="form-group mb-3">
                        <label class="form-control-label">RFID UID</label>
                        <input type="text" id="check-uid-input" class="form-control"
                            placeholder="Tap card or type UID..." autocomplete="off" inputmode="none" />
                    </div>
                    <button type="button" class="mb-2 btn btn-secondary btn-sm" id="check-uid-btn">
                        <i class="me-1 fa-solid fa-magnifying-glass"></i> Check
                    </button>
    ...
    <div class="modal fade" id="assignModal" tabindex="-1" aria-labelledby="assignModalLabel">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p class="mb-1 text-sm">Assigning card to user: <strong id="modal-user-code"></strong></p>
                    <p class="mb-3 text-muted text-xs">
                        <i class="me-1 text-success fa-solid fa-wifi"></i>
                        Tap the RFID card on the reader — the UID will fill in automatically via Pusher.
                        Or type it manually below.
                    </p>
                    <div class="form-group">
                        <label class="form-control-label">RFID UID</label>
                        <input type="text" id="rfid-uid-input" class="form-control"
                            placeholder="Tap card or type UID here..." autocomplete="off" />
                    </div>
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" id="save-rfid-btn" class="btn btn-primary">
                        <i class="me-1 fa-solid fa-floppy-disk"></i> Save
                    </button>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        var adminPusher = (function() {
            var pusher = new Pusher('{{ env('PUSHER_APP_KEY') }}', {
                cluster: '{{ env('PUSHER_APP_CLUSTER') }}',
            });
            var channel = pusher.subscribe('nfc-admin');
            channel.bind('card.tapped', function(data) {
                ...
                console.log('Card tapped:', uid, '| modalIsOpen:', modalIsOpen, '| currentUserId:',
                    currentUserId);
        ...
        function showToast(msg) {
            var $t = $(
                '<div class="position-fixed alert alert-warning alert-dismissible fade show" style="bottom:20px;right:20px;z-index:9999">' +
        ...
        $(document).on('click', '.unlink-btn', function() {
            ...
            $.ajax({
                ...
                success: function() {
                    $('.rfid-display-' + userId).html(
                        '<span class="text-muted fst-italic">Not assigned</span>');
                    $('[data-user-id="' + userId + '"].assign-btn').html(
                        '<i class="me-1 fa-solid fa-id-card"></i> Assign');
                    $btn.remove();
                ...
                success: function(res) {
                    if (res.linked) {
                        $('#check-card-result')
                            ...
                            .html(
                                '<i class="me-1 fa-solid fa-circle-xmark"></i> This card is already linked to user <strong>' +
                                res.user_code + '</strong>.' +
                                '<br><button class="mt-2 btn btn-sm btn-danger" id="check-unlink-btn" data-user-id="' +
                                res.user_id + '" data-user-code="' + res.user_code + '">' +
                                '<i class="me-1 fa-solid fa-link-slash"></i> Unlink from ' + res.user_code +
                                '</button>'
                            );
                    } else {
                        $('#check-card-result')
                            ...
                            .html(
                                '<i class="me-1 fa-solid fa-circle-check"></i> This card is <strong>not linked</strong> to any user.'
                                );
                    }
                    rfidBuffer = '';
        ...
        $(document).on('click', '#check-unlink-btn', function() {
            ...
            if (!confirm('Remove RFID card from user ' + userCode + '?')) return;
            var $btn = $(this);
            $btn.prop('disabled', true).html('<i class="me-1 fa-solid fa-spinner fa-spin"></i> Unlinking...');
            $.ajax({
                url: '{{ route('rfid.unlink') }}',
                type: 'POST',
                headers: {
                ...
                success: function() {
                    $('#check-card-result')
                        ...
                        .addClass('alert-success')
                        .html(
                            '<i class="me-1 fa-solid fa-circle-check"></i> Card has been <strong>unlinked</strong> from user <strong>' +
                            userCode + '</strong>.');
                ...
                error: function() {
                    $btn.prop('disabled', false).html(
                        '<i class="me-1 fa-solid fa-link-slash"></i> Unlink from ' + userCode);
                    $('#check-card-result').after(
                        '<div class="mt-1 alert alert-warning">Failed to unlink. Please try again.</div>'
                        );
                }
            });
        });
        ...
        function saveRfid(uid) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $display.html('<span class="bg-success badge">' + uid + '</span>');
                    $('[data-user-id="' + currentUserId + '"]').html(
                        '<i class="me-1 fa-solid fa-id-card"></i> Re-assign');
                    setTimeout(function() {
